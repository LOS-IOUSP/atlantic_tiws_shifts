...
# *- coding: utf-8 -*-
"""
Created on Sat Sep 9 14:45 2023
...
import numpy as np
import xarray as xr
from fir import maxvar, varexp, get_cLTA, \
	d2r, acorr, sombrero, obconvolve
...
class filter_wave:
	...
	def __init__(self, delx, delt, lat, z, T, L, z_ori):
		'''
		...
		self.z = z
		n = 1.25
		self.nx = int(round(abs(self.L)/(self.delx*111.195*np.cos(d2r(self.lat)))))
		self.nt = int(round(self.T/self.delt))
		if np.mod(self.nt, 2) == 0:
			self.nt += 1
		if np.mod(self.nx, 2) == 0:
			self.nx += 1
		print('[f_wave] numero de pontos em x = {}'.format(n*self.nx))
		print('[f_wave] numero de pontos em t = {}'.format(n*self.nt))
		self.f,x0,y0 = sombrero(n*self.nx, n*self.nt, np.sign(L)*self.nx, self.nt)
		fig,ax = plt.subplots(1,1, num=4,) #figsize=(11,5))
		ax.pcolormesh(x0,y0,self.f,cmap='PuOr')
		ax.set_xticklabels([])
		ax.set_yticklabels([])
		plt.show()

		try:
			self.zf = obconvolve(self.z, self.f)
			self.zf = maxvar(self.zf, self.z)*self.zf #garante ortogonalidade
		except OverflowError:
			print('OverflowError: cannot convert float infinity to integer')
			self.zf = z
			print('Refazer componente!')
		self.fvar = 100*varexp(z_ori, self.zf)
		print('Var Exp: {:.1f}'.format(self.fvar))

	def matriz_filtro(self):
		return self.zf
	...
	def get_param(self):
		A = np.sqrt(2)*self.zf.std()
		(npt, npx) = self.zf.shape
		#analise de FFT
		fft = FFT(self.zf, self.delt, self.delx, self.lat)
	...
	def show_hovs(self, tn, xn):

		fig, ax = plt.subplots(nrows=1, ncols=3, num=1, figsize=(8, 16), clear=True, sharey=True)

		res = self.z - self.zf
		ac = acorr(self.z)
		zk = {'Atual': self.z, 'Filtrado - VE: {:.1f}%'.format(self.fvar): self.zf, 'Resíduo':res} #'Autocorrelacao': ac,

		for i, key in enumerate(zk.keys()):
			#Hov original
			#Limits
			vmin = zk[key].mean() - 2*zk[key].std()
			vmax = zk[key].mean() + 2*zk[key].std()

			#Plotting
			pc = ax[i].pcolormesh(xn, tn, zk[key], vmin=vmin, vmax=vmax, cmap=cmo.curl, shading='auto')
			plt.colorbar(pc, ax=ax[i], orientation='horizontal', extend='both', pad=0.06, fraction=0.03, format='%d',
				ticks=np.linspace(vmin, vmax, 5))

			#Grid
			Ld = self.L/(111.195*np.cos(d2r(self.lat)))


			# faz o grid com L e T
			ax[i].xaxis.set_ticks(np.round(np.arange(xn[0], xn[-1], Ld*(-1))))
			ax[i].yaxis.set_ticks(np.round(np.arange(tn[0], tn[-1], self.T)))
			#ax[i].xaxis.set_major_formatter(StrMethodFormatter(r"{x:.0f}$^\circ$W"))
			ax[i].grid()
			if i == 0:
				ax[i].set_ylabel('Dias')
			ax[i].set_xlabel('0.25° de longitude')
...
class manual_filter(filter_wave):

	def __init__(self, delx, delt, lat, z, z_ori, tn, xn):
		plt.ion()
		...
		L0, T0, Te0, cp0, cpe0, temp_m, lon_m, temps, lons =  np.array([]), np.array([]), np.array([]), np.array([]), np.array([]), np.array([]),np.array([]),{},{}
		...
		while do_this_component:
			...
			cp0 = np.append(cp0, cp_crista)
			cp0 = np.append(cp0, cp_cavado)
			cpe0 = np.append(cpe0, np.std((np.diff(x0)/np.diff(t0))*111.195*np.cos(lat*np.pi/360)))
			cpe0 = np.append(cpe0, np.std((np.diff(x0)/np.diff(t1))*111.195*np.cos(lat*np.pi/360)))
			# save Period in days
		...
		filter_wave.__init__(self, delx, delt, lat, z, self.T1, self.L1, z_ori)
...
#class semi_auto_filter(filter_wave):
