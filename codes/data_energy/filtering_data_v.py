...
def calculate_mbporchunk(f1):
	chunk_shape = tuple(c[0] for c in f1.vo.chunks)
	n_elements = np.prod(chunk_shape)
	bits_per_element = np.dtype(f1.vo.dtype).itemsize * 8
...
f1 = xr.open_mfdataset('/data/anamariani/Data/GLORYS/uv_daily_mean/grid0p25/*.nc', chunks = {'time': 183, 'latitude': 5, 'longitude': 100, 'depth':1}, parallel=True)
_ = calculate_mbporchunk(f1)

#f1 = f1.isel(depth=[-2,-1])
depths = f1.depth.values

for i_d, depth in enumerate(depths):
	f = f1.isel(depth=i_d)
	print('Depth:{:.0f} - {}/{}'.format(depth,i_d,len(depths)))
	mean_t = f.vo.mean(dim='time')
	vo = f.vo - mean_t
	vo = vo.values
	print(mean_t)
	...
	m_ori = np.ones(f.vo.shape)*np.nan
	m_long = np.ones(f.vo.shape)*np.nan
	m_annual = np.ones(f.vo.shape)*np.nan
	#m_R_183 =  np.ones(f.vo.shape)*np.nan
	#m_R_121 = np.ones(f.vo.shape)*np.nan
	m_R_30 = np.ones(f.vo.shape)*np.nan
	#m_res = np.ones(f.vo.shape)*np.nan
		...
		if lat > -8. and lat < 8.:
			if lat > f_lat[f_lat>0].min() or lat < f_lat[f_lat<0].max():
				...
				if lat == 5. or lat == -5. or lat == 2.5 or lat == -2.5:
					...
					fig.savefig('/home/anamariani/Documents/Resultados/figuras_teste_GLORYS/teste_hovs_latitude{:.2f}_filtro_glo_v_D{:.0f}.png'.format(lat,depth), dpi=300)
					print('Salvo <3')

	m_ori = np.ma.masked_where(np.isnan(f.vo),m_ori)
	m_ori.set_fill_value(np.nan)
	m_long = np.ma.masked_where(np.isnan(f.vo),m_long)
	m_long.set_fill_value(np.nan)
	m_annual = np.ma.masked_where(np.isnan(f.vo),m_annual)
	m_annual.set_fill_value(np.nan)
	#m_R_183 = np.ma.masked_where(np.isnan(f.vo),m_R_183)
	#m_R_183.set_fill_value(np.nan)
	#m_R_121 = np.ma.masked_where(np.isnan(f.vo),m_R_121)
	#m_R_121.set_fill_value(np.nan)
	m_R_30 = np.ma.masked_where(np.isnan(f.vo),m_R_30)
	m_R_30.set_fill_value(np.nan)
	#m_res = np.ma.masked_where(np.isnan(f.vo),m_res)
	#m_res.set_fill_value(np.nan)

	print('Salvando dataset:')
	ds = xr.Dataset(
			data_vars=dict(v_ori=(['time', 'latitude', 'longitude'], m_ori),
				),
			coords=dict(latitude=(['latitude'], lats), longitude=(['longitude'], x), time=(['time'], jtime))
			)
	ds.to_netcdf('/data/anamariani/resultados_mestrado/energy_GLORYS/v_ori_filtered_D{:.2f}.nc'.format(depth))
	print('v_ori saved')
	ds = xr.Dataset(
			data_vars=dict(v_R_30= (['time', 'latitude', 'longitude'], m_R_30),
				),
			coords=dict(latitude=(['latitude'], lats), longitude=(['longitude'], x), time=(['time'], jtime))
			)
	ds.to_netcdf('/data/anamariani/resultados_mestrado/energy_GLORYS/v_30_filtered_D{:.2f}.nc'.format(depth))
	print('v_30 saved')
	ds = xr.Dataset(
			data_vars=dict(v_long= (['time', 'latitude', 'longitude'], m_long),
				v_annual= (['time', 'latitude', 'longitude'], m_annual),
				v_mean = (['latitude', 'longitude'], mean_t.values),
				),
			coords=dict(latitude=(['latitude'], lats), longitude=(['longitude'], x), time=(['time'], jtime))
			)
	ds.to_netcdf('/data/anamariani/resultados_mestrado/energy_GLORYS/v_annual_longterm_filtered_D{:.2f}.nc'.format(depth))
	print('v_long and annual saved')
	print('Done!')
