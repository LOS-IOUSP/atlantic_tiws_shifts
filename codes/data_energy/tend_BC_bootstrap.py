...
#Loading dataset
f = xr.open_dataset('/data/anamariani/resultados_mestrado/energy_GLORYS/Baroclinic_energy_sum.nc')
f = f.sel(longitude=slice(-60.,15.))
lons = f.longitude.values
lats = f.latitude.values
depth = f.depth.values[1::2]
time = f.time.values
faket = np.arange(1,len(time)+1,1)

for i_d, d in enumerate(depth):
    print('DEPTH: {:.0f} - {}/{}'.format(d,i_d+1,len(depth)))
    BT = f.BC_mean.sel(depth=d, method='nearest')*1000000 #microWm⁻³
    nt = len(time)
    ...
    print('Salvando <3')
    np.savez('/data/anamariani/resultados_mestrado/energy_GLORYS/mapa_tendencia_BC_D{:.0f}.npz'.format(d), lon=lon, lat=lat, tend_xy= tend_xy, mask_t=mask_t,  
        mean_xy=mean_xy)

    fig, ax = plot_map()

    fig.savefig('/home/anamariani/Documents/Resultados/figuras_teste_GLORYS/mapa_tendencia_BC_D{:.0f}.png'.format(d), bbox_inches='tight', dpi=300)
