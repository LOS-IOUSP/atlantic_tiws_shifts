...
def get_zg(lat,file):
    ...
    f = file.sel(latitude=lat)
    # convert from m to mm
    zg = 1000.0*(f.zos.values[:,:]-mean)
    zg = zg
...
f = xr.open_dataset('/data/anamariani/Data/GLORYS/ssh_daily_mean/grid0p25/cmems_mod_glo_phy_my_0.25deg_P1D-m_all.nc')
mean = f.zos.mean().values
print(mean)
...
m_ori = np.ones(f.zos.shape)*np.nan
m_long = np.ones(f.zos.shape)*np.nan
m_annual = np.ones(f.zos.shape)*np.nan
#m_R_183 =  np.ones(f.zos.shape)*np.nan
#m_R_121 = np.ones(f.zos.shape)*np.nan
m_R_30 = np.ones(f.zos.shape)*np.nan
#m_res = np.ones(f.zos.shape)*np.nan
	...
	if lat > -8. and lat < 8.:
		if lat > f_lat[f_lat>0].min() or lat < f_lat[f_lat<0].max():
			...
			if lat == 5. or lat == -5. or lat == 2.5 or lat == -2.5:
				...
				fig.savefig('/home/anamariani/Documents/Resultados/figuras_teste_GLORYS/teste_hovs_latitude{:.2f}_filtro_glo.png'.format(lat), dpi=300)
				print('Salvo <3')
			

m_ori = np.ma.masked_where(np.isnan(f.zos),m_ori)
m_ori.set_fill_value(np.nan)
m_long = np.ma.masked_where(np.isnan(f.zos),m_long)
m_long.set_fill_value(np.nan)
m_annual = np.ma.masked_where(np.isnan(f.zos),m_annual)
m_annual.set_fill_value(np.nan)
#m_R_183 = np.ma.masked_where(np.isnan(f.zos),m_R_183)
#m_R_183.set_fill_value(np.nan)
#m_R_121 = np.ma.masked_where(np.isnan(f.zos),m_R_121)
#m_R_121.set_fill_value(np.nan)
m_R_30 = np.ma.masked_where(np.isnan(f.zos),m_R_30)
m_R_30.set_fill_value(np.nan)
#m_res = np.ma.masked_where(np.isnan(f.zos),m_res)
#m_res.set_fill_value(np.nan)

print('Salvando dataset:')
ds = xr.Dataset(
		data_vars=dict(z_ori=(['time', 'latitude', 'longitude'], m_ori),
			),
		coords=dict(latitude=(['latitude'], lats), longitude=(['longitude'], x), time=(['time'], jtime))
		)
ds.to_netcdf('/data/anamariani/resultados_mestrado/energy_GLORYS/ssh_ori_filtered.nc')
print('z_ori saved')
ds = xr.Dataset(
		data_vars=dict(z_R_30= (['time', 'latitude', 'longitude'], m_R_30),
			),
		coords=dict(latitude=(['latitude'], lats), longitude=(['longitude'], x), time=(['time'], jtime))
		)
ds.to_netcdf('/data/anamariani/resultados_mestrado/energy_GLORYS/ssh_30_filtered.nc')
print('z_30 saved')
ds = xr.Dataset(
		data_vars=dict(z_long= (['time', 'latitude', 'longitude'], m_long),
			z_annual= (['time', 'latitude', 'longitude'], m_annual),
			),
		coords=dict(latitude=(['latitude'], lats), longitude=(['longitude'], x), time=(['time'], jtime))
		)
ds.to_netcdf('/data/anamariani/resultados_mestrado/energy_GLORYS/ssh_annual_longterm_filtered.nc')
print('z_long and annual saved')
print('Done!')
